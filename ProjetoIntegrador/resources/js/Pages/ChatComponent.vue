                ...
                <div class="w-9/12 flex flex-col justify-between overflow-y-scroll">
                    ...
                    <div v-for="message in messages" :key="message.id"
                        ...
                        <div class="w-full mb-3">
                            <p :class="(message.fk_id_user_from === $page.props.auth.user.id) ? 'messageFromMe' : 'messageToMe'"
    ...
    methods: {
        ...
        loadMessages: function (userId) {
            axios.get(`web/users/${userId}`).then(response => {
                ...
                // Adiciona o usuário na lista caso ainda não esteja
                if (!this.users.some(user => user.id === this.userActive.id)) {
                    this.users.push(this.userActive);
                }
            });
    ...
    mounted() {
        ...
        axios.get('web/users')
            .then(response => {
                ...
                // Se veio um usuário específico na URL, inicia o chat com ele
                if (targetUserId) {
